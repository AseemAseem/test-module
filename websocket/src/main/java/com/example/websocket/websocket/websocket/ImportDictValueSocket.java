...
//无前端代码，请用http://www.blue-zero.com/WebSocket/测试
...
public class ImportDictValueSocket {
    ...
    public void onMessage(String message, Session session) throws IOException, InterruptedException {
        System.out.println("来自客户端"+this.session.getRequestURI()+"的消息:" + message);
        ...
        sendOthers("来自客户端"+this.session.getRequestURI()+"的消息："+message);
    }
    ...
    public void sendOthers(String msg){
        for (ImportDictValueSocket item : webSocketSet) {
            if(this.session.equals(item.session)){
                break;
            }
            try {
